...
# Douglas et al. 2022 Script 3 (CMIP5)
...
SNpath = '/Volumes/GeoCCRI_01/users-data/douglahu/CMIP5_data/tas/Amon/01_MM_output/'
# Path for outputs
outPath = '/Volumes/GeoCCRI_01/users-data/douglahu/Scripts/CMIP5/'
...
# Note that these are the SSP scenarios
scen_long = [scenarios[0], scenarios[1], scenarios[4]]
...
SN_RCP4D = xr.DataArray(data=0.0, dims=['lat','lon','year','scenario'], 
                      coords=[popGridSSP4D.lat,popGridSSP4D.lon,popGridSSP4D.year,scen_short], 
                      name='SN') 
SN_RCP4D_16 = SN_RCP4D.copy(deep=True)
SN_RCP4D_84 = SN_RCP4D.copy(deep=True)

i = 0
for thisRCP in scen_short:
    print(thisRCP)
    SN_models = xr.open_dataarray(SNpath+'MM_dea2022_SN_'+thisRCP+'_allYears.nc')
    ...
    SN_RCP4D.loc[dict(scenario=thisRCP)]=SN_Out.values
    
    if avgType == 'median': 
        SN_Out16 = SN_Out16.transpose('lat','lon','year')
        SN_Out84 = SN_Out84.transpose('lat','lon','year')    
        SN_RCP4D_16.loc[dict(scenario=thisRCP)]=SN_Out16.values
        SN_RCP4D_84.loc[dict(scenario=thisRCP)]=SN_Out84.values
...
SN_RCP4D.to_netcdf(path=outPath+'dea2022_SN_RCP4D.nc')
...
for i in range(len(scen_short)):
    ...
    for thisThreshold in thresholds:
        ...
        for thisYear in years:
            ...
            thisSN = SN_RCP4D.loc[dict(year=thisYear, scenario=thisRCP)]
            pop_SN_over = thisPop.where(thisSN>thisThreshold)
            pop_exposed.loc[dict(year=thisYear, scenario=thisRCP, 
                                 threshold=thisThreshold)]=int(np.sum(pop_SN_over))
            
            thisSN_16 = SN_RCP4D_16.loc[dict(year=thisYear, scenario=thisRCP)]
            pop_SN_over_16 = thisPop.where(thisSN_16>thisThreshold)
            pop_exposed_16.loc[dict(year=thisYear, scenario=thisRCP, 
                                 threshold=thisThreshold)]=int(np.sum(pop_SN_over_16))
            
            thisSN_84 = SN_RCP4D_84.loc[dict(year=thisYear, scenario=thisRCP)]
            pop_SN_over_84 = thisPop.where(thisSN_84>thisThreshold)
            pop_exposed_84.loc[dict(year=thisYear, scenario=thisRCP, 
                                 threshold=thisThreshold)]=int(np.sum(pop_SN_over_84))
 
# Save the output as netCDFs for future use. 
pop_exposed.to_netcdf(path=outPath+'dea2022_pop_exposed_SN_RCP.nc')
pop_exposed_16.to_netcdf(path=outPath+'dea2022_pop_exposed_SN_RCP_16.nc')
pop_exposed_84.to_netcdf(path=outPath+'dea2022_pop_exposed_SN_RCP_84.nc')
...
for thisRCP in scen_short:
    ...
    # Regrid the SN data to the area grid with xESMF   
    ds_in = SN_Avg.to_dataset(promote_attrs=True)
...
for i in range(len(scen_short)):
    ...
    for thisThreshold in thresholds:
        ...
        for thisYear in years:
            ...
            area_exposed.loc[dict(year=thisYear, scenario=thisRCP, 
                                 threshold=thisThreshold)]=int(np.sum(area_SN_over))
            
            thisSN_16 = SN_RCP4D_16.loc[dict(year=thisYear, scenario=thisRCP)]
            area_SN_over_16 = thisarea.where(thisSN_16>thisThreshold)
            area_exposed_16.loc[dict(year=thisYear, scenario=thisRCP, 
                                 threshold=thisThreshold)]=int(np.sum(area_SN_over_16))
            
            thisSN_84 = SN_RCP4D_84.loc[dict(year=thisYear, scenario=thisRCP)]
            area_SN_over_84 = thisarea.where(thisSN_84>thisThreshold)
            area_exposed_84.loc[dict(year=thisYear, scenario=thisRCP, 
                                 threshold=thisThreshold)]=int(np.sum(area_SN_over_84))
 
# Save the output as netCDFs for future use. 
area_exposed.to_netcdf(path=outPath+'dea2022_area_exposed_SN_RCP.nc')
area_exposed_16.to_netcdf(path=outPath+'dea2022_area_exposed_SN_RCP_16.nc')
area_exposed_84.to_netcdf(path=outPath+'dea2022_area_exposed_SN_RCP_84.nc')