...
pop_exp = xr.open_dataarray(toePath+'dea2022_ECSfilter_pop_exposed_SN_SSP.nc')
pop_exp_16 = xr.open_dataarray(toePath+'dea2022_ECSfilter_pop_exposed_SN_SSP_16.nc')
pop_exp_84 = xr.open_dataarray(toePath+'dea2022_ECSfilter_pop_exposed_SN_SSP_84.nc')
...
for thisSSP in scen_short:
    thisMod = xr.open_dataarray(modPath+'MM_dea2022_ECSfilter_SN_'+thisSSP+'_allYears.nc')
...
for i in np.arange(0,len(scen_short)):
    if scen_short[i][0:4] != scen_long[i]:
        ...
        thisPlot.savefig('./Plots/dea2022_ECSfilter_pop-SN_TOE_'+scen_short[i]+'.svg')
...
# Load the time-of-emergence data (population exposed to S-N over threshold x)
area_exp = xr.open_dataarray(toePath+'dea2022_ECSfilter_area_exposed_SN_SSP.nc')
area_exp_16 = xr.open_dataarray(toePath+'dea2022_ECSfilter_area_exposed_SN_SSP_16.nc')
area_exp_84 = xr.open_dataarray(toePath+'dea2022_ECSfilter_area_exposed_SN_SSP_84.nc')
...
for i in np.arange(0,len(scen_short)):
    ...
    thisPlot.savefig('./Plots/dea2022_ECSfilter_area-SN_TOE_'+scen_short[i]+'.svg')
...
def medianCompPlot(areaArr, popArr, scen, figi, yoi_in):
    ...
    # Labels
    # Apply some filtering to minimise overlaps
    # yoi_use = yoi_in
    # if scen == 'ssp126':
    #     flag126 = True
    # else:
    ...
    # else:
    #     flag119 = False
    # if scen == 'ssp245' or scen == 'ssp370':
    #     yoi_use = yoi_in[0:2]
    # if scen == 'ssp585':
    #     yoi_use = yoi_in[0:1]
    #     # Plus a cheeky extra label for S/N3 at 2050 under ssp585:
    #     lab = str(yoi_in[1])
    #     xpt = popArr.loc[dict(scenario=scen, threshold=3, year=yoi_in[1])]*100
    #     ypt = areaArr.loc[dict(scenario=scen, threshold=3, year=yoi_in[1])]*100
    #     ax.annotate(lab, (xpt, ypt), textcoords="offset points", xytext=(5,-5), 
    #                 ha='left', color = palette[2]) 
    
    
    # for thisYear in yoi_use:
    #     lab = str(thisYear)
    #     if thisYear == 2100 and flag126:
    #         print('Skip due to overlap')
    #     else:
    ...
    #     if thisYear == 2100 and flag119:
    #         ax.annotate(lab, (xpt, ypt-2), textcoords="offset points", xytext=(5,-5), 
    ...
    # Create an xarray dataset for saving the coordinates of the plots and whether or 
    # not to print the labels.
    # lablBool = xr.Dataset(data_vars=dict(xpts=(['threshold','year'],np.zeros([4,len(yoi_in)])),
    #                                      ypts=(['threshold','year'],np.zeros([4,len(yoi_in)])),
    #                                      plotOn=(['threshold','year'],np.array(np.ones([4,len(yoi_in)]),dtype='bool'))),
    #                       coords=dict(threshold = [1,2,3,5], 
    #                                   year = yoi_in))
...
for i in np.arange(0,len(scen_short)):
    ...
    thisPlot.savefig('./Plots/dea2022_ECSfilter_area-pop-exposed_'+scen_short[i]+'.svg')
...
dummy = xr.open_dataarray(modPath+'MM_dea2022_ECSfilter_SN_'+str(Yn1)+'-'+str(Yn2-1)+'_ssp126.nc')
...
for thisSSP in scen_short:
    ...
    try:
        # Noise
        N_models = xr.open_dataarray(modPath+'MM_dea2022_ECSfilter_noise_'+thisSSP+'.nc')
        noiseAvg = N_models.median('model', keep_attrs=True, skipna=True)
        N_SSP.loc[dict(scenario=thisSSP)]= noiseAvg.values
        
        # Signal
        S_models = xr.open_dataarray(modPath+'MM_dea2022_ECSfilter_signal_'+str(Yn1)+'-'+str(Yn2-1)+'_'+thisSSP+'.nc')
        signalAvg = S_models.median('model', keep_attrs=True, skipna=True)
        S_SSP.loc[dict(scenario=thisSSP)]= signalAvg.values
        
        # S/N
        SN_models = xr.open_dataarray(modPath+'MM_dea2022_ECSfilter_SN_'+str(Yn1)+'-'+str(Yn2-1)+'_'+thisSSP+'.nc')
...
gridPlot.savefig('./Plots/dea2022_ECSfilter_SSP_SN-grid-all_'+str(Yn1)+'-'+str(Yn2-1)+'.svg', bbox_inches='tight', dpi=300)
...
gridPlot.savefig('./Plots/dea2022_RCP_SN-grid-all_'+str(Yn1)+'-'+str(Yn2-1)+'.pdf', bbox_inches='tight', dpi=300)
...
gridPlot.savefig('./Plots/dea2022_ECSfilter_N-S-SN_SSP-RCP_'+str(Yn1)+'-'+str(Yn2-1)+'.pdf', bbox_inches='tight', dpi=300)