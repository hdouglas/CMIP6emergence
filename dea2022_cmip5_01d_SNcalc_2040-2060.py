...
# Douglas et al. 2022 Script 1 for CMIP5 series of data
...
# Inputs: raw netcdf files from the CMIP5 database
...
from dea2022_fn_cmip5_signoise import signoise_calc
...
scen_short = ['rcp26','rcp45','rcp85'] #'rcp60',
...
for thisRCP in scen_short:
    ...
    for thisDir in dirList:
        if thisDir == '.DS_Store':
            ...
            fileList = os.listdir()
            try: 
                fileList.index('dea2022_allEns_signal_'+str(POI[0])+'-'+str(POI[1])+'_'+thisRCP+'.ncdummy')
                ...
                try:
                    ...
                    ensembleList = np.sort(os.listdir(homedir+'/'+variable_id+'/'+table_id+'/'+thisDir+'/'+thisRCP))
                    ...
                    # that's what's used if there's a conflict between historical and RCP grids.
                    ...
                    dummy = xr.open_dataset(homedir+'/'+variable_id+'/'+table_id+
                                              ...
                                              '/'+dum_file, use_cftime=usingCFtime)
                    try:
                        print(len(dummy.lat))
                    except:
                    ...
                    for thisVariant in ensembleList:
                        ...
                        data = signoise_calc(thisDir, thisRCP, framework, POI[0], POI[1], thisVariant)
                        #print('It worked!')
                    ...
                    noiseArr.to_netcdf(path='dea2022_allEns_noise_'+thisRCP+'.nc')
                    signalArr.to_netcdf(path='dea2022_allEns_signal_'+str(POI[0])+'-'+str(POI[1])+'_'+thisRCP+'.nc')
                    SN_Arr.to_netcdf(path='dea2022_allEns_SN_'+str(POI[0])+'-'+str(POI[1])+'_'+thisRCP+'.nc')
                    signalArr_allYears.loc[dict(year=years)].to_netcdf(path='dea2022_allEns_signal_'+thisRCP+'_allYears.nc')
                    SN_Arr_allYears.loc[dict(year=years)].to_netcdf(path='dea2022_allEns_SN_'+thisRCP+'_allYears.nc')
