...
from dea2022_fn_cmip5_gmst import gmst_calc
...
for thisRCP in scen_short:
    ...
    for thisDir in dirList:
        if thisDir == '.DS_Store':
            ...
            fileList = os.listdir()
            try: 
                fileList.index('dea2022_allEns_gmst_'+thisRCP+'_allYears.nc')
                ...
                try:
                    ...
                    # Set up dataArray for storing the gmst
                    ...
                    for thisVariant in ensembleList:
                        ...
                        data = gmst_calc(thisDir, thisRCP, 2100, thisVariant)
                        print('It worked!')
                        ...
                        print('cp-a1')
                        print(data['tasAverageRel'].year)
                        gmst_allYears.loc[dict(variant=thisVariant)] = data['tasAverageRel'].loc[dict(year=years)]
                        print('cp-a2')
                    # Save the output
                    print('cp-b1')
                    gmst_allYears.loc[dict(year=years)].to_netcdf(path='dea2022_allEns_gmst_'+thisRCP+'_allYears.nc')
                    print('cp-b2')
                except:
                    pass
            os.chdir(homedir+'/'+variable_id+'/'+table_id)
